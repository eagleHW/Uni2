function enviaDatos(){
    ...
    $("#mensaje_datos").text("\u231B Procesando").css("color", "black");
    
    $.post("guardarCambiosUsuario",
           $("#form_datos").serialize(),
           function(data){
               
               if(data === 'ok'){
                    $("#mensaje_datos").text("\u2714 Tus datos han sido actualizados").css("color", "green");
               }else{
                    $("#mensaje_datos").text("\u2718 Fallo validación en servidor").css("color", "red");
               }
               
           })
             .fail(function(){
                 $("#mensaje_datos").text("\u2718 Error de conexión con el servidor, intenta más tarde").css("color", "red");
           });
               
}

function enviaContrasena(){
    
    $("#mensaje_contrasena").text("\u231B Procesando").css("color", "black");
    
    $.post("guardarCambiosContrasena",
          $("#form_contrasena").serialize(),
          function(data){
           
             if(data === 'ok'){
                 $("#mensaje_contrasena").text("\u2714 Tu contraseña ha sido actualizada").css("color", "green");
             }else{
                 $("#mensaje_contrasena").text("\u2718 Contraseña anterior incorrecta").css("color", "red");
               } 
...
function limpiaContrasena(){
    ...
    document.getElementById("contrasena_nueva").classList.remove("error");
}
...
function revisaContrasena(){
    var contraseniaUno = document.getElementById("contrasena_nueva").value;
